"""
run_dmavae.py — Run DMAVAE on ONE replication (single SLURM array task).

Usage:
    python run_dmavae.py <task_id>     # task_id in 1..800

Reads CSVs that R/generate_data.R wrote, runs DMAVAE, writes ONE row to:
    $PROJECT_ROOT/performance_dmavae/<scenario>/rep<rep>.csv
plus a per-rep loss curve to:
    $PROJECT_ROOT/performance_dmavae/<scenario>/loss_rep<rep>.csv
"""
