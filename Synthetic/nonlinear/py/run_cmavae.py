"""
run_cmavae.py — Run CMAVAE on ONE replication (single SLURM array task).

Usage:
    python run_cmavae.py <task_id>     # task_id in 1..800

Writes ONE row to:
    $PROJECT_ROOT/performance_cmavae/<scenario>/rep<rep>.csv
plus loss curve to:
    $PROJECT_ROOT/performance_cmavae/<scenario>/loss_rep<rep>.csv
"""
...
from load_datasets import load_data, get_true_effects
from cmavae_gpu    import CMA_VAE

logging.getLogger("pyro").setLevel(logging.WARNING)

# ---- Settings (kept identical to DMAVAE for fair comparison) ----
NUM_EPOCHS       = 700
BATCH_SIZE       = 128
LR               = 1e-4
LR_DECAY         = 0.01
...
def main():
    if len(sys.argv) < 2:
        sys.exit("Usage: python run_cmavae.py <task_id>")
    ...
    perf_dir  = os.path.join(project_root, "performance_cmavae")

    idx = task_id_to_indices(task_id)
    print(f"[run_cmavae] task={task_id}  scenario={idx['name']}  rep={idx['rep']}",
          flush=True)

    pyro.enable_validation(__debug__)
    torch.set_default_tensor_type("torch.FloatTensor")

    x, m, t, y = load_data(input_dir, idx["N"], idx["n_X"], idx["sigma2_x"],
                            idx["rep"])
    assert x.shape[1] == idx["n_X"], (
        f"Expected n_X={idx['n_X']} columns, got {x.shape[1]}.")

    pyro.clear_param_store()
    model = CMA_VAE(
        feature_dim = idx["n_X"],
        latent_dim  = 5,        # CMAVAE paper default for synthetic data
        hidden_dim  = 128,
        num_layers  = 4,
        num_samples = 10,
    )
    ...
    n_batches = max(1, (idx["N"] + BATCH_SIZE - 1) // BATCH_SIZE)
    ...
    truth = get_true_effects(input_dir, idx["N"], idx["n_X"], idx["sigma2_x"],
                              idx["rep"])
    ...
    print(f"[run_cmavae] done. converged={conv['converged']}  "
