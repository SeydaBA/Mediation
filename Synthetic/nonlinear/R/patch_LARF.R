# Replace LARF::Generate.Powers — avoids stats::polym integer overflow at large p.
...
.patched_Generate.Powers <- function(X, lambda) {
  ...
  # Enumerate all p-tuples of non-negative integers with total degree 1..lambda
  ...
  # Drop perfectly collinear columns (matches original behavior)
  tab <- cor(Full.Data) == 1
...
# ALSO patch causalweight's imports environment — the parent of its namespace.
# Unqualified calls inside causalweight (e.g. hdmedalt calls Generate.Powers
# without LARF:: prefix) resolve here.
.patch_causalweight_imports <- function() {
  ...
  if ("Generate.Powers" %in% ls(envir = imp_env, all.names = TRUE)) {
    ...
    cat("  NOTE: Generate.Powers not in causalweight imports env\n")
  }
}
